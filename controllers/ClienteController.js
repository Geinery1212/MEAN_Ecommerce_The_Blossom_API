...
registro_cliente_tienda = async function (req, res) {
    ...
    clientes_arr = await Cliente.find({
        email: data.email
    });
...
const listar_productos_destacados_publico = async function (req, res) {
    let reg = await Producto_etiqueta.find({
        etiqueta: "61a390d39b40d02e0cb9d789"
    }).populate('producto');
    res.status(200).send({
        data: reg
    });
...
const registro_direccion_cliente = async function (req, res) {
    if (req.user) {
        ...
        if (data.principal) {
            let direcciones = await Direccion.find({
...
const eliminar_direccion_cliente = async function (req, res) {
    if (req.user) {
        var id = req.params['id'];
        let direcciones = await Direccion.findByIdAndUpdate({
            _id: id
        }, {
            status: false
        });
...
const listar_productos_publico2 = async function (req, res) {
    let arr_data = [];
    let reg = await Producto.find({
        estado: 'Publicado'
    }).sort({
...
const agregar_carrito_cliente = async function (req, res) {
    if (req.user) {
        ...
        if (data.cantidad <= variedad.stock) {
            ...
            res.status(200).send({
                data: undefined,
                message: 'Stock insuficiente, ingrese otra cantidad'
            });
        }
...
const agregar_carrito_cliente = async function (req, res) {
    if (req.user) {
        let data = req.body;
        var entrar = 'No entrar';
        let reg = false;
        let variedad = await Variedad.findById({ _id: data.variedad });
        let carrito = await Carrito.find();
        datacantidad = data.cantidad;
        carrito.forEach((c) => {
            if (data.producto == c.producto && data.variedad == c.variedad) {
                datacantidad = data.cantidad + c.cantidad;
            }
        });
        console.log(datacantidad);
        if (datacantidad <= variedad.stock) {
            if (carrito.length == 0) {
                entrar = 'Entrar';
            } else {
                entrar = 'No entrar';
            ...
            let bandera = false;
            carrito.forEach(async (c, i) => {
                if (data.producto == c.producto && data.variedad == c.variedad) {
                    bandera = true;
                    reg = await Carrito.findByIdAndUpdate({ _id: c._id }, { cantidad: c.cantidad + data.cantidad });
                    entrar = 'No entrar';
                } else {
                    if (carrito.length == (i + 1) && bandera == false) {
                        entrar = 'Entrar';
...
const eliminar_carrito_cliente = async function (req, res) {
    if (req.user) {
        let id = req.params['id'];
        let reg = await Carrito.findByIdAndRemove({
            _id: id
        });
        res.status(200).send({
            data: reg
    ...
    try {
        ...
        readHTMLFile(process.cwd() + '/mails/email_pedido.html', (err, html) => {
            ...
            var mailOptions = {
                ...
                subject: 'Gracias por tu orden, The Blossom.',
                html: htmlToSend
            };
...
const obtener_reviews_cliente = async function (req, res) {
    if (req.user) {
        ...
        let reg = await Review.find({
            cliente: id
        }).populate('cliente').populate('producto');
        res.status(200).send({
            data: reg
        });
